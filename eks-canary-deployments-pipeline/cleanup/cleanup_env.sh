...
images=(nginx node postgres express-node react-app)
...
stacks=(eks-pipeline-react-app eks-pipeline-express-node)
...
parameters=(eks-canary-express-node-version eks-canary-react-app-version)
